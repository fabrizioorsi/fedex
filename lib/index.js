...
function FedEx(args) {
    ...
    var endpoints = {
        ...
        rate: {
            f:       buildRateRequest,
            r:       handleRateResponse,
            wsdl:    'RateService_v24.wsdl',
            version: { ServiceId: 'crs', Major: 24, Intermediate: 0, Minor: 0 }
        },
        // track: {
        //     f:       buildRateRequest,
        //     r:       handleRateResponse,
        //     wsdl:    'TrackService_v16.wsdl',
        //     version: { ServiceId: 'crs', Major: 24, Intermediate: 0, Minor: 0 }
        // }
    }
   ...
   function buildAuthentication(resource) {
    ...
    // build a new shipment request
    function buildRateRequest(data, resource, callback) {
        soap.createClient(path.join(__dirname, 'wsdl/'+$scope.options.env, resource.wsdl), function(err, client) {
            if (err) {
                return callback(err, null);
            }
    ...
    // handle shipment request response
    function handleRateResponse(res, callback) {
        return callback(null, res);
    }
